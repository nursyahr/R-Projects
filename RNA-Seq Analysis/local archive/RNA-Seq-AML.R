...
EnhancedVolcano( wholeRes, lab = wholeRes$SYMBOL, 
                 ...
                 pCutoff = 0.05, FCcutoff = 0.5, pointSize = 2.0, 
                 labSize = 3.0,
...
top_20_DE <- sigRes %>%
  ...
  dplyr::filter(absLFC > 1.5 & padj < 0.05) %>%
  arrange(desc(absLFC)) %>%
  top_n(20)
...
topDE_up <- sigRes %>%
  dplyr::filter(padj < .05 & log2FoldChange > 1.2)

topDE_down <- sigRes %>%
  dplyr::filter(padj < .05 & log2FoldChange < -1.5) %>%
  arrange(log2FoldChange) 

utils::write.csv(topDE_up, file = "over_expressed_genes.csv")
utils::write.csv(topDE_down, file = "under_expressed_genes.csv")
